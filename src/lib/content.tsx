...
export const faqItems = [
  { q: "Is AIDAL a law firm? Does using it make us legally compliant?", a: <>No. AIDAL provides <strong>technical audit infrastructure</strong> — independently verifiable decision records, cryptographic proofs, and automatic compliance field-checking based on the actual regulation text. Confirm full compliance with a licensed lawyer.</> },
  { q: "Will AIDAL audit logs be accepted as evidence in regulatory proceedings?", a: <>The log is <strong>independently verifiable</strong> by any third party without relying on AIDAL&apos;s servers — a stronger evidentiary position than a database printout, which no outside party can check at all. Under the hood that's a SHA-256 hash chain, the same structure used in blockchain forensics accepted by financial regulators globally. Whether a specific regulator in a specific proceeding accepts it depends on jurisdiction; consult a licensed compliance lawyer.</> },
  ...
  { q: "What happens if AIDAL shuts down?", a: <>Your records stay verifiable. Export them at any time and check them with our open-source offline verifier — it uses only the Python standard library, needs no AIDAL account, and makes no network calls. That check survives us disappearing entirely. Separately, a GPG-signed master hash is published daily to a <strong>public GitHub repository</strong>; that adds corroboration, but be aware AIDAL holds the credentials that write to it — see <a href="https://github.com/widjajaanthony24-svg/AIDAL/blob/main/EVIDENCE_SPEC.md" target="_blank" rel="noopener" className="underline underline-offset-2">§8.2 of the Evidence Specification</a> for exactly what the anchor does and doesn&apos;t prove.</> },
  { q: "We already log decisions in our database. Why AIDAL?", a: <>Your database can be edited — a regulator knows this. AIDAL gives you a record where tampering is <strong>mathematically detectable</strong> instead of merely policy-forbidden (SHA-256 hash chain, under the hood). It also adds AI explanations, compliance checking, and a regulator-ready PDF.</> },
  ...
  { q: "How long does integration actually take?", a: <><strong>Under 30 minutes.</strong> One request after your model runs, no SDK, no infrastructure — the mechanism is simple on purpose so the evidence layer isn't the hard part of your compliance work. <a href="https://aidal-production.up.railway.app/docs" target="_blank" rel="noopener" className="underline underline-offset-2">API docs →</a></> },
  { q: "We're not regulated yet. Should we still use AIDAL?", a: <>Regulation is coming. Retrofitting an audit trail after a regulator asks for it is significantly harder. The sandbox tier costs nothing to start on.</> },
];
...
export const pricingTiers = [
  {
    ...
    features: ["Full API access", "AI-generated explanations", "Independently verifiable audit log", "7 jurisdiction modules", "Public verify page"],
